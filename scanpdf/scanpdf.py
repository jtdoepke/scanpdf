...
class ScanPdf(object):
    ...
    def run_scan(self):
        device = (
            os.environ["SCANBD_DEVICE"] if self.scandevice is None else self.scandevice
        )
        self.cmd('logger -t "scanbd: " "Begin of scan "')
        c = [
            'SANE_CONFIG_DIR="%s"' % self.sanedir,
            "scanadf",
            '-d "%s"' % device,
            '--source "ADF Duplex"',
            "--mode Color",
    ...
    def parse_dimensions(self, result):
        ...
        mCropDim = re.compile(
            """\s*(?P<filename>[\d\w\[_\/\\\.]+)\s+\w+\s+(?P<X>\d+)x(?P<Y>\d+)\s+"""
        )
    ...
    def is_blank(self, filename):
        ...
        c = (
            "convert %s -shave %sx%s -density %s -adaptive-resize 65%% -virtual-pixel White -blur 0x15 -fuzz 15%% -trim info:"
            % (filename, self.dpi, self.dpi, int(self.dpi / 2))
        )
        result = self.cmd(c)
        ...
        if x > 0 and y > 0:
            ...
            logging.debug(
                "Could not find dimensions in output of imagemagick for cropping"
            )
            return False
    ...
    def _page_to_bw(self, page):
        ...
        cmd = (
            "convert %s +dither -density %s -colors 16 -colors 4 -colorspace gray -normalize %s_bw"
            % (page, self.dpi, page)
        )
        out = self.cmd(cmd)
        # Remove the old file
    ...
    def _is_color(self, filename):
        ...
        """
        cmd = (
            "convert %s -density %s -adaptive-resize 35%% -colors 8 -depth 8 -format %%c histogram:info:-"
            % (filename, int(self.dpi / 3))
        )
        out = self.cmd(cmd)
        mLine = re.compile(
            r"""\s*(?P<count>\d+):\s*\(\s*(?P<R>\d+),\s*(?P<G>\d+),\s*(?P<B>\d+).+"""
        )
        colors = []
    ...
    def get_options(self, argv):
        ...
        if (
            argv["--device"]
            and argv["--device"] != "the SCANBD_DEVICE environment variable"
        ):
        ...
        if argv["scan"]:
            if os.path.exists(self.tmp_dir):
                self._error(
                    "Temporary output directory %s already exists!" % self.tmp_dir
                )
            else:
